...
const TriedUpdate = () => {
    ...
    useEffect(() => {
        axios.get(
            `http://localhost:8080/api/tried/detail/${triedIdx}`, { headers: { 'Authorization': `Bearer ${sessionStorage.getItem('token')}` } })
            .then(response => {
            ...
            <div className="triedDetail-container">
                <form onSubmit={handlerSubmit} action="" method="POST" id="frm" name="frm">
                    ...
                    <div className="update-img">
                        {uploadImg.length == 0 && <img src={`http://localhost:8080/api/getimage/${filename}`} style={{ width: '500px' }} />}
                    </div>
                    <div className="update-img">

...
function Thumb(props) {
  ...
  useEffect(() => {
    ...
    //해당 글 좋아요 수 조회
    axios.get(`http://192.168.0.4:8080/api/listidealreal/detail/getlike/${idealrealIdx}`)
      .then(response => {
    ...
    axios.get(`http://192.168.0.4:8080/api/listidealreal/detail/likecheck/${idealrealIdx}/${loginId}`)
      .then(response => {
        console.log(response);
  ...
  const LikeCountHandler = () => {
    ...
    if (likeCheck == 0) {
      ...
      axios.post(`http://192.168.0.4:8080/api/${idealrealIdx}/like`,
        data, headers)
        .then(response => {
          console.log(response);
          setLikeCheck(1);
      ...
      setLikeCount(prev => prev - 1)
      axios.delete(`http://192.168.0.4:8080/api/${idealrealIdx}/unlike`,
        { data })
        .then(response => {
          console.log(response);
          setLikeCheck(0);

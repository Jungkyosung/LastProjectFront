...
const Exchange = (props) => {
  ...
  useEffect( () => {
    axios.get(`http://192.168.0.39:8080/api/exchangeList`, {headers : header})
    .then((response) => {
  ...
  const handlerSelect = (e) => {
    ...
    let arrExchange = exchangeList.filter( array => array.exchangeNationShort == e.target.value );
    ...
    <div className="Experience__ExchangeBox">
      <div className="Experience__Input" >
        ...
        <select onChange={(e)=>handlerSelect(e)} value={selectExchange}>
          <option value="" disabled selected>국적을 선택하세요</option>
          {exchangeList.map((ex) => (
      ...
      <div className="Experience__ExchangeKR">
        <p>원화 총 금액</p> 
          <ul>{priceTotal}won</ul>
      </div>


      {/* 환율 변환 총 금액 */}
      <div className="Experience__ExchangeOther">
          <ul>{(priceTotal/totalExchange).toFixed(2)} 총 금액 </ul>
      </div>
    </div>
  );
};
